...
package com.gorillalogic.utils {
    ...
    public class AttributeFinder {
        ...
        public function getStyles(target:DisplayObject):ArrayCollection {
            ...
            var stylesMetadata:Array = FlexSpyFrameworkMetadata.getClassStyles(targetStyleClient);

            for each (var styleMetadata:FlexSpyStyleMetadata in stylesMetadata) {
        ...
        static public function getStyleValue(target:DisplayObject, styleName:String):String {
            var targetStyleClient:IStyleClient = target as IStyleClient;
            var style:AttributeVO;
            var name:String;
            var styleNames:Array = new Array();
            ...
            //          }
            throw new Error("Style name " + styleName + " not found.");
        }

        static private function createStyle(stylableObject:IStyleClient, styleMetadata:FlexSpyStyleMetadata):AttributeVO {
            var style:AttributeVO = new AttributeVO(styleMetadata.name, null, AttributeVO.STYLE);
            return style;
        }

        static private function inspectStyle(stylableObject:IStyleClient, name:String, styles:Object, styleNames:Array, inherited:Boolean):AttributeVO {
